...
public class PcaContainerAPI {
    ...
    public static Map<Item, Integer> getAllContainerItems() {
        ...
        if (world == null || client.player == null) return result;

        // 扫描方块实体容器
        for (BlockEntity be : getLoadedBlockEntities(world)) {
        ...
        // 扫描实体容器
        for (Entity entity : getLoadedContainerEntities(world)) {
     ...
     */
    private static List<BlockEntity> getLoadedBlockEntities(World world) {
        ...
        for (int cx = -radius; cx <= radius; cx++) {
            for (int cz = -radius; cz <= radius; cz++) {
                ...
                if (world.isChunkLoaded(chunkX, chunkZ)) {
                    world.getChunk(chunkX, chunkZ).getBlockEntities().forEach((pos, be) -> {
     ...
     * 获取世界中所有已加载的容器实体。
     */
    private static List<Entity> getLoadedContainerEntities(World world) {
     ...
     */
    private static void addInventoryItems(Map<Item, Integer> result, Inventory inv) {
        for (int i = 0; i < inv.size(); i++) {
            ItemStack stack = inv.getStack(i);
            if (!stack.isEmpty()) {
