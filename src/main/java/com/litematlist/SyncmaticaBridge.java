...
public final class SyncmaticaBridge {
    ...
    private static LitematicReader.MaterialEntry toEntry(String itemIdStr, String variant, int missingAmount) {
        ...
        Item item = parseItem(itemIdStr);
        String variantSuffix = "";
        if (item == null && variant != null && !variant.isEmpty() && variant.length() <= 64) {
            // variant 非空时尝试 <itemId>_<variant> 组合（如氧化变体）
            item = parseItem(itemIdStr + "_" + variant);
            variantSuffix = " (" + variant + ")";
        }
        if (item == null || item == Items.AIR) {
            return null;
        }
        String blockName = item.getName(new ItemStack(item)).getString() + variantSuffix;
    ...
    private static Item parseItem(String id) {
        ...
        return BuiltInRegistries.ITEM.get(identifier).map(Holder::value).orElse(null);
    }
}