...
public class MaterialListScreen extends GuiBase {
    ...
    private static final Identifier SYNC_LOGO_TEXTURE = Identifier.fromNamespaceAndPath("litematlist", "textures/gui/syncmatica_logo.png");
    ...
    public void initGui() {
        ...
        int newProjW = this.font.width(newProjLabel) + 16;
        ...
        this.addButton(newProjBtn, (IButtonActionListener) (b, mb) ->
                Minecraft.getInstance().setScreenAndShow(new ProjectCreateScreen(this)));
        ...
        this.addButton(importBtn, (IButtonActionListener) (b, mb) -> {
            ...
            Minecraft.getInstance().setScreenAndShow(new TxtBrowserScreen(this));
        });
        ...
        this.addButton(addBtn, (IButtonActionListener) (b, mb) -> {
            ...
            Minecraft.getInstance().setScreenAndShow(new SchematicBrowserScreen(this));
        });
        ...
        // 「上传区域」按钮（主页面右上角，作者署名下方；仅在配置开启时显示）
        if (Configs.Generic.SHOW_UPLOAD_AREA_BUTTON.getBooleanValue()) {
            String uploadAreaLabel = I18n.tr("litematlist.button.upload_area");
            int uploadAreaW = this.font.width(uploadAreaLabel) + 16;
            uploadAreaBtnX = this.width - uploadAreaW - 10;
            uploadAreaBtnY = 26;
            uploadAreaBtnW = uploadAreaW;
            uploadAreaBtnH = 20;
            ButtonGeneric uploadAreaBtn = new ButtonGeneric(uploadAreaBtnX, uploadAreaBtnY, uploadAreaBtnW, uploadAreaBtnH, uploadAreaLabel);
            uploadAreaBtn.setRenderDefaultBackground(true);
            this.addButton(uploadAreaBtn, (IButtonActionListener) (b, mb) ->
                    Minecraft.getInstance().setScreenAndShow(new UploadAreaScreen(this)));
            // HUD界面设置按钮（上传区域左侧）
            String hudConfigLabel = I18n.tr("litematlist.button.hud_config");
            int hudConfigW = this.font.width(hudConfigLabel) + 16;
            ButtonGeneric hudConfigBtn = new ButtonGeneric(this.width - uploadAreaW - hudConfigW - 14, 26, hudConfigW, 20, hudConfigLabel);
            ...
            this.addButton(hudConfigBtn, (IButtonActionListener) (b, mb) -> {
                ...
                Minecraft.getInstance().setScreenAndShow(new HudConfigScreen(this));
            });
        }
        ...
        for (int i = scrollOffset; i < maxIdx; i++) {
            ...
            if (entry.isProject) {
                ...
                this.addButton(openBtn, (IButtonActionListener) (b, mb) -> {
                    ...
                    Minecraft.getInstance().setScreenAndShow(new ProjectScreen(this, projectId, children));
                });
                ...
                this.addButton(rawMaterialsBtn, (IButtonActionListener) (b, mb) -> {
                    List<LoadedEntry> children = getProjectEntries(projectId);
                    Minecraft.getInstance().setScreenAndShow(new RawMaterialScreen(this, projectId, null));
                });
                ...
                this.addButton(viewTotalBtn, (IButtonActionListener) (b, mb) -> {
                    List<LoadedEntry> children = getProjectEntries(projectId);
                    Minecraft.getInstance().setScreenAndShow(new ProjectSummaryScreen(this, projectId, children));
                });
            ...
            if (SyncmaticaBridge.isSyncPath(entry.filePath())) {
                ...
                rawMaterialsBtn.setRenderDefaultBackground(true);
                this.addButton(rawMaterialsBtn, (IButtonActionListener) (b, mb) ->
                        Minecraft.getInstance().setScreenAndShow(new RawMaterialScreen(this, entry.name(), entry.filePath())));
                ...
                this.addButton(viewBtn, (IButtonActionListener) (b, mb) ->
                        Minecraft.getInstance().setScreenAndShow(new MaterialDetailScreen(this, entry.name(), entry.filePath())));
            ...
            this.addButton(addToProjBtn, (IButtonActionListener) (b, mb) -> {
                ...
                Minecraft.getInstance().setScreenAndShow(new ProjectSelectScreen(this, moveIdx, projects));
            });
            ...
            if (!isTxtEntry(entry) && !isJsonEntry(entry) && !isCsvEntry(entry)) {
                ...
                this.addButton(renameBtn, (IButtonActionListener) (b, mb) -> {
                    Minecraft.getInstance().setScreenAndShow(new RenameScreen(this, renameIdx, entry.name()));
                });
            }
    ...
    private List<LoadedEntry> getDisplayEntries() {
        ...
        if (nameCmp != null) {
            materials.sort((a, b) -> {
                int c = Boolean.compare(!a.pinned, !b.pinned);
                if (c != 0) return c;
                return nameCmp != null ? nameCmp.compare(a, b) : 0;
            });
        } else {
    ...
    public void extractRenderState(GuiGraphicsExtractor gfx, int mouseX, int mouseY, float partialTicks) {
        ...
        drawListBackground(ctx);
        drawHeader(ctx, mouseX, mouseY);
        drawUploadedRedBackgrounds(ctx); // 红色标识在按钮下方
        super.extractRenderState(gfx, mouseX, mouseY, partialTicks);
        drawListContent(ctx, mouseX, mouseY);
        drawHeaderText(ctx); // 表头文字在最顶层
        drawSyncTooltip(ctx, mouseX, mouseY);
        drawImportBtnTooltip(ctx, mouseX, mouseY);
        drawAddBtnTooltip(ctx, mouseX, mouseY);
        drawPreviewTooltip(ctx, mouseX, mouseY);
        drawPreviewDisabledTooltip(ctx, mouseX, mouseY);
        drawPinTooltip(ctx, mouseX, mouseY);
        drawUploadAreaTooltip(ctx, mouseX, mouseY);
    }
    ...
    private int getHeaderTextWidth() {
        ...
        return this.font.width(fullText);
    }
    ...
    private void drawListBackground(GuiContext ctx) {
        int visibleRows = this.getVisibleRows();
        int listBottom = LIST_TOP + visibleRows * ROW_HEIGHT;
        ...
        for (int i = scrollOffset; i < maxIdx; i++) {
            int rowY = LIST_TOP + (i - scrollOffset) * ROW_HEIGHT;
            int color = (i == draggedIndex && isDragging) ? 0x60FFFF00 :
                    ((i - scrollOffset) % 2 == 0) ? 0x20FFFFFF : 0x10FFFFFF;
    ...
    private void drawListContent(GuiContext ctx, int mouseX, int mouseY) {
        ...
        int watermarkW = this.font.width(watermark);
        int watermarkX = this.width - watermarkW - 28;
        ctx.drawString(this.font, watermark, watermarkX, 12, 0xFF55FFFF);
        // 作者署名悬停提示
        if (mouseX >= watermarkX && mouseX <= watermarkX + watermarkW && mouseY >= 12 && mouseY <= 22) {
            drawTooltipBox(ctx, I18n.tr("litematlist.tooltip.author_signature"), mouseX, mouseY);
        }
        List<LoadedEntry> displayForCount = getDisplayEntries();
        drawCenteredText(ctx,
                I18n.tr("litematlist.info.total_lists", displayForCount.size()), this.width / 2, 24, 0xFFFFFFFF);
        ...
        if (errorMessage != null) {
            if (System.currentTimeMillis() - errorMessageTime > 3000) {
                ...
                drawCenteredText(ctx, errorMessage, this.width / 2, this.height - 56, 0xFFFF5555);
            }
        }

        if (displayForCount.isEmpty()) {
            drawCenteredText(ctx,
                    I18n.tr("litematlist.info.no_lists"),
        ...
        for (int i = scrollOffset; i < maxIdx; i++) {
            ...
            if (entry.isProject) {
                // 项目文件夹：文件夹图标
                drawTextureIcon(ctx, FOLDER_ICON, PIN_ICON_X, rowY + PIN_ICON_Y_OFFSET, PIN_ICON_SIZE, PIN_ICON_SIZE);
                ...
                ctx.drawString(this.font, folderName, 34, rowY + 5, 0xFFFFFFFF);
                if (mouseX >= 34 && mouseX <= 34 + this.font.width(folderName)
                        && mouseY >= rowY && mouseY < rowY + ROW_HEIGHT) {
                    drawTooltipBox(ctx, entry.name(), mouseX, mouseY);
                }
                continue;
            }
            ...
            if (SyncmaticaBridge.isSyncPath(entry.filePath())) {
                drawPinIcon(ctx, SYNC_LOGO_TEXTURE, pinX, pinY);
            } else {
                ...
                drawPinIcon(ctx, pinTexture, pinX, pinY);

                // 聚焦外框：悬停时叠加显示
                if (pinHovered) {
                    drawPinIcon(ctx, PIN_FOCUSED, pinX, pinY);
                }
            }
            ...
            // 被折叠时悬停显示完整名称
            if (truncated && mouseX >= 34 && mouseX <= 34 + this.font.width(displayName)
                    && mouseY >= rowY && mouseY < rowY + ROW_HEIGHT) {
                drawTooltipBox(ctx, entry.name(), mouseX, mouseY);
            }
        }
    ...
    private void drawSyncTooltip(GuiContext ctx, int mouseX, int mouseY) {
        if (mouseX >= syncBtnX && mouseX <= syncBtnX + syncBtnW &&
            ...
            for (String line : lines) {
                tipW = Math.max(tipW, this.font.width(line) + 8);
            }
            int tipH = lines.length * 12 + 4;
            int tipX = mouseX + 12;
            int tipY = mouseY - 20 - (lines.length - 1) * 12;
            ...
            int lineY = tipY + 4;
            for (String line : lines) {
                ctx.drawString(this.font, line, tipX + 4, lineY, 0xFFFFFFFF);
                lineY += 12;
            }
        }
    }
    ...
    /** 绘制"导入列表文件"按钮的悬停提示 */
    private void drawImportBtnTooltip(GuiContext ctx, int mouseX, int mouseY) {
        if (mouseX >= importBtnX && mouseX <= importBtnX + importBtnW &&
                mouseY >= importBtnY && mouseY <= importBtnY + importBtnH) {
            drawTooltipBox(ctx, I18n.tr("litematlist.tooltip.import_list_file"), mouseX, mouseY);
        }
    }

    /** 绘制"添加材料列表"按钮的悬停提示 */
    private void drawAddBtnTooltip(GuiContext ctx, int mouseX, int mouseY) {
        if (mouseX >= addBtnX && mouseX <= addBtnX + addBtnW &&
                mouseY >= addBtnY && mouseY <= addBtnY + addBtnH) {
            drawTooltipBox(ctx, I18n.tr("litematlist.tooltip.add_material_list"), mouseX, mouseY);
        }
    }

    /** 绘制"预览原理图"按钮的悬停提示 */
    private void drawPreviewTooltip(GuiContext ctx, int mouseX, int mouseY) {
        for (PreviewBtnInfo info : previewBtnInfos) {
            if (mouseX >= info.x && mouseX <= info.x + info.w &&
                    mouseY >= info.y && mouseY <= info.y + info.h) {
                drawTooltipBox(ctx, I18n.tr("litematlist.tooltip.fullscreen_preview") + " (ESC)", mouseX, mouseY);
                return;
            }
        }
    }

    private void drawPreviewDisabledTooltip(GuiContext ctx, int mouseX, int mouseY) {
        for (PreviewBtnInfo info : previewBtnInfos) {
    ...
    /** 绘制置顶图标的悬停提示 */
    private void drawPinTooltip(GuiContext ctx, int mouseX, int mouseY) {
        List<LoadedEntry> display = getDisplayEntries();
        ...
        if (row >= 0 && row < display.size() && isInPinArea(mouseX)) {
            ...
            drawTooltipBox(ctx, tip, mouseX, mouseY);
        }
    }

    /** 绘制已上传条目的红色背景（整行，在按钮层下方） */
    private void drawUploadedRedBackgrounds(GuiContext ctx) {
        int visibleRows = this.getVisibleRows();
        List<LoadedEntry> display = getDisplayEntries();
        int maxIdx = Math.min(display.size(), scrollOffset + visibleRows);
        for (int i = scrollOffset; i < maxIdx; i++) {
            ...
            int rowY = LIST_TOP + (i - scrollOffset) * ROW_HEIGHT;
            if (entry.uploaded) {
                ctx.fill(10, rowY, this.width - 10, rowY + ROW_HEIGHT, 0xFFAB0508);
            }
        }
    }

    /** 绘制「上传区域」按钮的悬停提示 */
    private void drawUploadAreaTooltip(GuiContext ctx, int mouseX, int mouseY) {
        if (Configs.Generic.SHOW_UPLOAD_AREA_BUTTON.getBooleanValue()
                && mouseX >= uploadAreaBtnX && mouseX <= uploadAreaBtnX + uploadAreaBtnW
                && mouseY >= uploadAreaBtnY && mouseY <= uploadAreaBtnY + uploadAreaBtnH) {
            drawTooltipBox(ctx, I18n.tr("litematlist.tooltip.upload_area"), mouseX, mouseY);
        }
    }
    ...
    private void drawPinIcon(GuiContext ctx, Identifier textureId, int x, int y) {
        ...
        BlitRenderState blit = new BlitRenderState(
                ...
                texSetup,
                new Matrix3x2f(),
                x, y, x + PIN_ICON_SIZE, y + PIN_ICON_SIZE,
                0.0f, 1.0f, 0.0f, 1.0f,
                0xFFFFFFFF,
                null
        );
    ...
    public static class LoadedEntry {
        ...
        boolean pinned;
        public String projectId; // 所属项目ID（null=根列表）
        public boolean isProject; // 是否为项目文件夹
