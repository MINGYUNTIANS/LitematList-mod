...
public class MultiBlockPickerScreen extends GuiBase {
    ...
    public void initGui() {
        ...
        this.searchField = new EditBox(this.font, fieldX + 4, searchY + 2, 94, 12, null);
    ...
    private void filterItems(String text) {
        ...
        for (Identifier id : BuiltInRegistries.ITEM.keySet()) {
            ...
            if (PinyinSearch.matches(text, new ItemStack(item).getHoverName().getString(), id.toString())) {
                filteredItems.add(item);
            }
        }
        // 按创造模式物品栏顺序排序（数字ID=注册顺序）
    ...
    public void extractRenderState(GuiGraphicsExtractor drawContext, int mouseX, int mouseY, float partialTicks) {
        ...
        if (searchField != null) {
            ...
            if (txt.isEmpty()) {
                this.drawString(ctx, I18n.tr("litematlist.search.placeholder"), fieldX + 4, searchY + 2, 0x80FFFFFF);
            } else {
                this.drawString(ctx, txt, fieldX + 4, searchY + 2, 0xFFFFFFFF);
            }
            if (searchField.isFocused() && System.currentTimeMillis() % 1000 < 500) {
                ...
                int cx = fieldX + 4 + this.getStringWidth(before);
        ...
        this.drawStringWithShadow(ctx, I18n.tr("litematlist.label.selected_count", selectedItems.size()), previewLeft, 24, 0xFFFFFFFF);
        ...
        for (int i = previewStartIdx; i < selectedList.size(); i++) {
            ...
            drawContext.fakeItem(new ItemStack(item), px + 1, py + 1);
            ...
            this.drawStringWithShadow(ctx, "×", crossX + 1, crossY - 1, 0xFFFFFFFF);
