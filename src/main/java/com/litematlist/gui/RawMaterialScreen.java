...
public class RawMaterialScreen extends GuiBase {
    ...
    // ---- 树状图搜索 ----
    private static final Identifier SEARCH_ICON = Identifier.parse("litematlist:textures/gui/search.png");
    ...
    private static final int DRAG_THRESHOLD = 16; // 位移平方超过阈值判定为拖动

    // ---- 树状图整体缩放（开关在【配置】-【通用】；【-】百分比【+】显示在右下角） ----
    private static final Identifier ZOOM_IN_ICON = Identifier.parse("litematlist:textures/gui/zoom/zoom_in.png");
    private static final Identifier ZOOM_OUT_ICON = Identifier.parse("litematlist:textures/gui/zoom/zoom_out.png");
    ...
    // 树状图水平滚动下限：缩放状态开启且树整体小于视口时为负数（允许整体平移），其余状态为 0
    ...
    // ---- 折叠/展开图标 ----
    private static final Identifier EXPAND_NORMAL = Identifier.parse("litematlist:textures/gui/expand/expand_normal.png");
    private static final Identifier EXPAND_HOVERED = Identifier.parse("litematlist:textures/gui/expand/expand_hovered.png");
    private static final Identifier COLLAPSE_NORMAL = Identifier.parse("litematlist:textures/gui/expand/collapse_normal.png");
    private static final Identifier COLLAPSE_HOVERED = Identifier.parse("litematlist:textures/gui/expand/collapse_hovered.png");
    private static final int EXPAND_ICON_SIZE = 12;

    // ---- 替换图标（通配物品） ----
    private static final Identifier GEAR_ICON = Identifier.parse("litematlist:textures/gui/gear.png");
    ...
    // ---- 层数加减图标 ----
    private static final Identifier DEPTH_NORMAL = Identifier.parse("litematlist:textures/gui/depth/depth_normal.png");
    private static final Identifier DEPTH_HOVERED = Identifier.parse("litematlist:textures/gui/depth/depth_hovered.png");
    private static final Identifier DEPTH_LOCKED = Identifier.parse("litematlist:textures/gui/depth/depth_locked.png");
    ...
    public void initGui() {
        ...
        if (depthField == null) {
            ...
            this.depthField.setBordered(false);
        ...
        this.addButton(rawConfigBtn, (IButtonActionListener) (b, mb) -> {
            Minecraft.getInstance().setScreen(new RawMaterialConfigScreen(this));
        });
    ...
    private void loadOrAnalyze() {
        ...
        Set<net.minecraft.world.item.Item> materialIgnored;
        ...
        for (MaterialListScreen.LoadedEntry child : children) {
            ...
            if (childEntries != null) {
                ...
                for (MaterialEntry entry : childEntries) {
                    ...
                    String key = BuiltInRegistries.ITEM.getKey(effectiveItem).toString();
    ...
    public void extractRenderState(GuiGraphicsExtractor drawContext, int mouseX, int mouseY, float partialTicks) {
        ...
        // 创造模式排序按钮
        {
            ...
            ctx.drawString(this.font, state, textStartX + prefixW, 39, stateColor);
        }
        // 是否同步替换按钮
        {
        ...
        if (searchIconHovered) {
            ctx.fill(searchIconX - 1, searchIconY - 1,
                    searchIconX + SEARCH_ICON_SIZE + 1, searchIconY + SEARCH_ICON_SIZE + 1, 0x40FFFFFF);
        }
        // 21x21 纹理按 1:1 绘制
        drawIconTexture(ctx, SEARCH_ICON, searchIconX, searchIconY, SEARCH_ICON_SIZE);

        if (analyzing) {
            ctx.drawCenteredString(this.font, I18n.tr("litematlist.raw_materials.analyzing"),
                    this.width / 2, (this.height - 100) / 2, 0xFFFFFF55);
            return;
        }

        if (treeRoots.isEmpty()) {
            ctx.drawCenteredString(this.font, I18n.tr("litematlist.raw_materials.empty"),
            ...
            // 绘制连接线
            for (NodeLayout root : rootLayouts) {
                renderConnectors(ctx, root, wMouseX, wMouseY);
            }

            // 绘制节点
            for (NodeLayout root : rootLayouts) {
                renderNodeTree(ctx, root, wMouseX, wMouseY, treeAreaBottom);
            }
            ...
            if (hoveredRecipeNode != null && hoveredNode == null) {
                ...
                if (rn.alternativeRecipeTypes != null && !rn.alternativeRecipeTypes.isEmpty()) {
                    ctx.getGuiGraphics().setTooltipForNextFrame(this.font,
                            Component.literal(I18n.tr("litematlist.recipe.alt_available")), mouseX, mouseY);
                }
            }
            ...
            if (mouseX >= exportRawBtnX && mouseX < exportRawBtnX + exportRawBtnW
                ...
                ctx.getGuiGraphics().setComponentTooltipForNextFrame(this.font, tooltipLines, mouseX, mouseY);
            }
        }
        ...
        ctx.drawString(this.font, depthLabel, 18, depthCtrlY + 4, 0xFFFFFFFF);
        ...
        if (depthField != null) {
            ...
            ctx.drawString(this.font, txt, fieldX + 3, depthCtrlY + 4, 0xFFFFFFFF);
        ...
        drawIconTexture(ctx, depthIcon, depthIconX, depthCtrlY, DEPTH_ICON_SIZE);

        // ---- 树状图搜索控件（搜索框/信息行/导航按钮，在树之上渲染） ----
        if (this.searchVisible) {
            // 输入框
            ctx.fill(searchFieldX - 2, searchFieldY - 1,
                    searchFieldX + SEARCH_FIELD_W + 2, searchFieldY + SEARCH_FIELD_H + 1, 0xFFFFFFFF);
            ctx.fill(searchFieldX - 1, searchFieldY,
            ...
            if (searchField != null) {
                ...
                if (this.font.width(txt) > availW) {
                    shown = this.font.substrByWidth(Component.literal(txt),
                            availW - this.font.width("…")).getString() + "…";
                }
                ctx.drawString(this.font, shown, searchFieldX + 3, searchFieldY + 3, 0xFFFFFFFF);
                if (searchField.isFocused() && System.currentTimeMillis() % 1000 < 500) {
                    ...
                    ctx.fill(cx, searchFieldY + 2, cx + 1, searchFieldY + SEARCH_FIELD_H - 2, 0xFFFFFFFF);
                }
            }
            ...
            ctx.drawString(this.font, info, searchNavX, searchNavY + 4, 0xFFFFFFFF);
            ...
            if (total > 0) {
                ...
                drawSearchNavButton(ctx, searchUpBtnX, searchBtnY, "↑", mouseX, mouseY);
                drawSearchNavButton(ctx, searchDownBtnX, searchBtnY, "↓", mouseX, mouseY);
            }
        }

        // ---- 右下角缩放控制条（【-】百分比【+】；【配置】-【通用】开关） ----
        if (this.treeZoomEnabled) {
            drawZoomBar(ctx, mouseX, mouseY);
        }

        // 右下角斜体提示：界面可用鼠标拖动
        String dragHint = I18n.tr("litematlist.raw_materials.drag_hint");
        int hintW = this.font.width(dragHint);
        ctx.drawString(this.font, Component.literal(dragHint).withStyle(style -> style.withItalic(true)), this.width - hintW - 10, this.height - 18, 0x60FFFFFF);

        // ---- 滚动条渲染 ----
        renderScrollbars(ctx, mouseX, mouseY);

        // ---- 左下角统计信息 ----
        int[] counts = countNodes();
        String stats = I18n.tr("litematlist.raw_materials.stats", counts[0], counts[1]);
        ctx.drawString(this.font, stats, 10, this.height - 21, 0x80FFFFFF);
    }

    // ==================== 节点渲染 ====================

    private void renderNodeTree(GuiContext ctx, NodeLayout layout,
        ...
        if (item != null) {
            ctx.fakeItem(new ItemStack(item), screenX + 6, screenY + 2);
        }

        String fullName = layout.node.itemId;
        if (item != null) {
            fullName = item.getName(new ItemStack(item)).getString();
        }
        ...
        String displayName = this.font.substrByWidth(Component.literal(fullName), maxAvailWidth).getString();
        if (displayName.length() < fullName.length()) {
            String withEllipsis = this.font.substrByWidth(Component.literal(fullName), maxAvailWidth - this.font.width("…")).getString();
        ...
        ctx.drawString(this.font, displayName, nameStartX, screenY + 5, 0xFFFFFFFF);
        ctx.drawString(this.font, countStr, countX, screenY + 5, 0xFFAAAAAA);
        ...
        if (isWildcard && gearIconX > 0) {
            ...
            drawIconTexture(ctx, GEAR_ICON, gearIconX, gearY, GEAR_ICON_SIZE);
        }
        ...
        if (!layout.node.isLeaf()) {
            ...
            drawIconTexture(ctx, icon, expandIconX, iconY, EXPAND_ICON_SIZE);
        }
        ...
        if (item != null) {
            ...
            if (nameHovered) {
                ...
                ctx.getGuiGraphics().setTooltipForNextFrame(this.font, Component.literal(tip), frameScreenMouseX, frameScreenMouseY);
    ...
    private void drawIconTexture(GuiContext ctx, Identifier textureId, int x, int y, int size) {
        Pair<GpuTextureView, GpuSampler> pair = ctx.bindTexture(textureId);
        if (pair != null && pair.getLeft() != null) {
            ctx.blit(pair.getLeft(), pair.getRight(), x, y, x + size, y + size, 0.0f, 1.0f, 0.0f, 1.0f);
        }
    }

    private void renderConnectors(GuiContext ctx, NodeLayout layout,
        ...
        if (recipeType != null && !recipeType.isEmpty() && !"base".equals(recipeType)) {
            ...
            if (iconX + RECIPE_ICON_SIZE >= 0 && iconX <= cullRightW()
                ...
                if (stationItem != null) {
                    ctx.fakeItem(stationItem, iconX, iconY);
                }
    ...
    /** 画水平线，裁剪到可视区域 */
    private void drawHLine(GuiGraphicsExtractor ctx, int x1, int x2, int y, int color) {
    ...
    /** 画竖线，裁剪到可视区域 */
    private void drawVLine(GuiGraphicsExtractor ctx, int x, int y1, int y2, int color) {
    ...
    /** 绘制右下角缩放控制条 */
    private void drawZoomBar(GuiContext ctx, int mouseX, int mouseY) {
        ...
        ctx.drawString(this.font, pct, pctX, zoomBarY + (ZOOM_BTN_SIZE - 8) / 2, 0xFFFFFFFF);
        ...
        drawIconTexture(ctx, ZOOM_OUT_ICON, zoomOutBtnX, zoomBarY, ZOOM_BTN_SIZE);
        ...
        drawIconTexture(ctx, ZOOM_IN_ICON, zoomInBtnX, zoomBarY, ZOOM_BTN_SIZE);
    }

    /** 绘制正方形导航按钮（↑/↓），悬停时变亮 */
    private void drawSearchNavButton(GuiContext ctx, int bx, int by, String arrow,
                                     int mouseX, int mouseY) {
        ...
        ctx.drawCenteredString(this.font, arrow, bx + SEARCH_NAV_BTN_SIZE / 2,
                by + (SEARCH_NAV_BTN_SIZE - 8) / 2, 0xFFFFFFFF);
    }

    /** 每帧刷新匹配列表：收集树中所有可见叶子节点，按屏幕"从左到右、从上到下"排序后过滤出匹配项 */
        ...
        if (searchVisible && !kw.isEmpty()) {
            ...
            for (NodeLayout node : allNodes) {
                ...
                if (numericKw) {
                    ...
                    if (item != null) name = item.getName(new ItemStack(item)).getString();
    ...
    private void collectLeafNodesRecursive(NodeLayout layout, List<String[]> out) {
        if (layout.collapsed || layout.children.isEmpty()) {
            // 折叠节点或无子节点 = 叶子节点
            Item item = BuiltInRegistries.ITEM.get(Identifier.parse(layout.node.itemId)).map(Holder::value).orElse(null);
            String itemName = item != null ? item.getName(new ItemStack(item)).getString() : layout.node.itemId;
            out.add(new String[] {
                layout.node.itemId,
                itemName,
    ...
    private void sendExportMessage(String fmt, String filename, Path exportPath) {
        ...
        if (client.player != null) {
            Component link = Component.literal(filename)
                    .withStyle(style -> style
                            .withClickEvent(new ClickEvent.OpenFile(exportPath))
                            .withUnderlined(true).withColor(0x55FFFF));
    ...
    private Set<Item> getCompatibleItems(String itemId) {
        ...
        Item targetItem = BuiltInRegistries.ITEM.get(Identifier.parse(itemId)).map(Holder::value).orElse(null);
        ...
        var registry = Minecraft.getInstance().level != null
                ...
                : null;
        if (registry != null) {
            var itemTags = registry.lookup(Registries.ITEM);
            if (itemTags.isPresent()) {
                var tagRegistry = itemTags.get();
                tagRegistry.getTags().forEach(tag -> {
    ...
    private void addItemById(Set<Item> collected, String itemId) {
        Item item = BuiltInRegistries.ITEM.get(Identifier.parse(itemId)).map(Holder::value).orElse(null);
        if (item != null) {
            collected.add(item);
        }
    }
        ...
        for (Item item : items) {
            ...
            if (typeSuffix.equals("wooden_") && id.startsWith("minecraft:wooden_")) {
                ...
                if (typeSuffix.equals("_slab")) {
                    ...
                    if (BuiltInRegistries.ITEM.containsKey(Identifier.parse(plankId))) {
                        filtered.add(item);
                    }
                } else {
                    filtered.add(item);
     ...
     * 渲染垂直和水平滚动条。
     */
    private void renderScrollbars(GuiContext ctx, int mouseX, int mouseY) {
        int treeAreaTop = CONTROLS_BOTTOM;
        int treeAreaBottom = this.height - TREE_BOTTOM_OFFSET;
        int treeAreaHeight = treeAreaBottom - treeAreaTop;
        ...
        if (maxScrollX > 0) {
            ...
            int thumbX = trackX + (int) ((double) scrollX / maxScrollX * (trackW - thumbW));
