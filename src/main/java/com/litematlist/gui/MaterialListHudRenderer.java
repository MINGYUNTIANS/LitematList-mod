...
public class MaterialListHudRenderer {
     ...
     */
    private static void drawHudCount(GuiContext ctx, net.minecraft.client.gui.Font font, int count, int leftX, int y) {
        ...
        if (count <= 0 || (!MaterialDetailScreen.persistedHudShowBox && !MaterialDetailScreen.persistedHudShowGroup)) {
            ctx.drawString(font, countStr, leftX, y, mainColor);
            return;
        }
        ...
        ctx.drawString(font, countStr, cx, y, mainColor);
        cx += font.width(countStr);

        cx = appendColored(ctx, font, "(", cx, y, 0xFFFFFFFF);
        if (showBox) {
            cx = appendColored(ctx, font, String.format(Locale.ROOT, "%.2f%s",
        ...
        appendColored(ctx, font, ")", cx, y, 0xFFFFFFFF);
    }

    /** 以指定颜色绘制一段文字，并返回下一个片段的起始 X 坐标 */
    private static int appendColored(GuiContext ctx, net.minecraft.client.gui.Font font, String text, int x, int y, int color) {
    ...
    public static void render(GuiGraphicsExtractor gfx) {
        ...
        // 优化 #4：所有行都渲染（含 missing==0），visibleLine 独立计数
        if (itemScale == 1.0f) {
            int visibleLine = 0;
            for (int i = startIdx; i < endIdx; i++) {
                HudItem hi = hudItems.get(i);
                ...
                drawHudCount(ctx, client.font, hi.missing, x + cachedTotalW - 8 - cachedCountMaxW, lineY + 1);
            }
        } else {
            int visibleLine = 0;
            for (int i = startIdx; i < endIdx; i++) {
