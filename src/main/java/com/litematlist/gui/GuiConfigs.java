...
public class GuiConfigs extends GuiConfigsBase
{
    ...
    /**
     * 使用自定义列表部件，覆写过滤候选生成逻辑：
     * 在 maLib 默认候选（内部名/显示名）基础上追加显示名与注释的小写文本及拼音全拼/首拼，
     * 使配置界面搜索支持拼音全拼、首拼及模糊搜索。
     */
    @Override
    protected WidgetListConfigOptions createListWidget(int listX, int listY)
    {
        return new SearchableListWidget(listX, listY, this.getBrowserWidth(), this.getBrowserHeight(),
    ...
    /**
     * 配置列表部件：仅覆写过滤候选生成逻辑，其余行为保持默认。
     */
    private static class SearchableListWidget extends WidgetListConfigOptions
    {
        public SearchableListWidget(int x, int y, int width, int height, int configWidth,
