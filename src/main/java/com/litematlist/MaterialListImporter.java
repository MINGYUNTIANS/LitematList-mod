...
public class MaterialListImporter {
     ...
     * 先按物品ID查找，再按名称查找。
     */
    private static Item findItemByIdOrName(String idOrName) {
        Identifier id = Identifier.tryParse(idOrName);
        if (id != null) {
            Item item = Registries.ITEM.get(id);
            if (item != Items.AIR) return item;
        }
        return findItem(idOrName);
    }
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (inDataSection && line.startsWith("|")) {
                    ...
                    String missingStr = "0";
                    // 原材料导出只有 3 列 (Item | Count | Recipe)，Litematica 导出有 4+ 列
                    // 我们只需要 Item 和 Count，Missing 默认为 0
                    if (parts.length >= 3) {
                        itemName = parts[1].trim();
                        totalStr = parts[2].trim();
                        if (!rawFormat && parts.length >= 4) {
                            // 4+列格式（Litematica 导出），第三列是 Missing
                            missingStr = parts[3].trim();
                        }
                    }
                    if (itemName == null || itemName.isEmpty() || itemName.equals("Item")) continue;

                    try {
                        int total = Integer.parseInt(totalStr);
                        int missing = !missingStr.isEmpty() ? Integer.parseInt(missingStr) : 0;
                        if (total > 0) {
                            Item item = findItem(itemName);
                            if (item == Items.AIR) {
                                // 尝试用 ID 匹配（原材料导出第一列是物品名称，可能和 ID 不同名）
                                // 如果是原材料导出，第一列是物品名称，第二列是数量，第三列是配方类型
                                // 我们用名称匹配，如果找不到就跳过
            ...
            for (Map.Entry<Item, Integer> entry : itemCounts.entrySet()) {
                ...
                materials.add(new LitematicReader.MaterialEntry(
                        item, itemNames.getOrDefault(item, item.getName().getString()), entry.getValue(), itemMissing.getOrDefault(item, 0)));
            }
    ...
    private static ImportResult importFromCsv(Path filePath) {
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            while ((line = reader.readLine()) != null) {
                ...
                String itemId = unquote(parts[0]);
                String itemName = unquote(parts[1]);
                if (itemId.isEmpty()) continue;

                try {
                    int total = Integer.parseInt(unquote(parts[2]).trim());
                    int missing = 0;
                    // 原材料导出 CSV: ItemID,Name,Count,Recipe (4列，Missing 默认为 0)
                    // 原始导出 CSV: Item,Name,Total,Missing (4列)
                    // 兼容两种格式
                    if (parts.length >= 4) {
                        String fourth = unquote(parts[3]).trim();
                        try {
                            if (!fourth.isEmpty() && !fourth.equalsIgnoreCase("recipe")) {
                                missing = Integer.parseInt(fourth);
                            }
                        } catch (NumberFormatException ignored) {
                            // 如果第四列不是数字（如配方类型），missing 保持 0
                        }
                    }
                    if (total > 0) {
                        ...
                        if (item == Items.AIR) {
                            LOGGER.warn("跳过未匹配物品: {} ({})", itemId, itemName);
                            skippedCount++;
                            continue;
                        }
                        String displayName = itemName.isEmpty() ? item.getName().getString() : itemName;
                        itemCounts.merge(item, total, Integer::sum);
                        itemMissing.merge(item, missing, Integer::sum);
                        itemNames.putIfAbsent(item, displayName);
                        totalItems += total;
                        totalMissing += missing;
                    }
                } catch (NumberFormatException ignored) {
    ...
    private static PreviewInfo parseCsvPreview(Path filePath) {
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    // 原材料导出 CSV: ItemID,Name,Count,Recipe → 数量在第 2 列
                    // 原始导出 CSV: Item,Name,Total,Missing → 数量在第 2 列
                    int total = Integer.parseInt(unquote(parts[2]).trim());
                    int missing = 0;
                    if (parts.length >= 4) {
                        String fourth = unquote(parts[3]).trim();
    ...
    private static PreviewInfo parseTxtPreview(Path filePath) {
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (line.contains("原理图") && line.contains("原材料")) {
                    int start = line.indexOf("'");
                    int end = line.lastIndexOf("'");
                    if (start >= 0 && end > start) {
                        schematicName = line.substring(start + 1, end);
    ...
    public static ImportResult importFromJson(Path filePath) {
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            for (Map.Entry<Item, Integer> entry : itemCounts.entrySet()) {
                Item item = entry.getKey();
                materials.add(new LitematicReader.MaterialEntry(
                        item, item.getName().getString(), entry.getValue()));
            }
            materials.sort((a, b) -> Integer.compare(b.totalCount(), a.totalCount()));
    ...
    private static void ensureItemCache() {
        ...
        for (Map.Entry<String, Item> entry : itemByNameCache.entrySet()) {
            if (count++ >= 10) break;
            LOGGER.info("  缓存: '{}' -> {}", entry.getKey(), Registries.ITEM.getId(entry.getValue()));
        }
    }
    ...
    private static Item findItem(String name) {
        ...
        if (item != null) {
            LOGGER.debug("物品匹配成功: '{}' -> {}", name, Registries.ITEM.getId(item));
            return item;
        }
