    ...
    public static List<SchematicInfo> getLoadedSchematics() {
        ...
        try {
            ...
            for (SchematicPlacement placement : placements) {
                ...
                Path filePath = placement.getSchematicFile();
                if (filePath != null) {
    ...
    public static LayerFilter computeRenderLayerFilter(Path filePath) {
        ...
        try {
            ...
            for (SchematicPlacement placement : DataManager.getSchematicPlacementManager().getAllSchematicsPlacements()) {
                Path placementFile = placement.getSchematicFile();
                ...
                LayerFilter filter = new LayerFilter(axis.ordinal(),
                        range.getMinLayerBoundary() - originK, range.getMaxLayerBoundary() - originK);
                LitematListMod.LOGGER.info("[LitematicaBridge] 渲染层规则: 轴={} 世界区间 [{}..{}] 原点k={} → 相对层级 [{}..{}]",
                        axis, range.getMinLayerBoundary(), range.getMaxLayerBoundary(), originK, filter.min(), filter.max());
                return filter;
            }
