...
public class RawMaterialTreeAnalyzer {
    ...
    private RecipeTreeNode traceItemInternal(Holder<Item> item, int count,
        ...
        for (List<RecipeBookUtils.Type> typeGroup : orderedPriority) {
            ...
            // 遍历查找首个带合成需求的条目（26.x 部分display条目缺失craftingRequirements）
            ...
            for (Pair<RecipeDisplayId, RecipeDisplayEntry> pair : lookup) {
                ...
                if (candidate.craftingRequirements().isEmpty()) continue;
                if (preferShaped
        ...
        for (var entry : dedup.entrySet()) {
            ...
            if (WILDCARD_MATERIAL_IDS.contains(childId)) {
                ...
                if (!childId.equals(originalId)) {
                    Item replacedItem = BuiltInRegistries.ITEM.get(Identifier.parse(childId)).map(Holder::value).orElse(null);
    ...
    private RecipeTreeNode processRecipe(List<Ingredient> ingredients, int resultCount, int total,
        ...
        Map<Holder<Item>, Integer> dedup = new LinkedHashMap<>();
        for (Ingredient ing : ingredients) {
            SlotDisplay slotDisplay = ing.display();
            List<ItemStack> displayStacks = slotDisplay.resolveForStacks(map);
            if (displayStacks.isEmpty()) continue;
    ...
    private RecipeFallback findRecipeFromManager(Item targetItem) {
        try {
            ...
            var recipeManager = client.level.recipeAccess();
            ...
            for (Object entryObj : recipeEntries) {
                ...
                LitematListMod.LOGGER.debug("[RawMaterialTreeAnalyzer] RecipeManager找到配方: {} -> {} (类型: {})",
                        BuiltInRegistries.ITEM.getId(targetItem), recipe.getClass().getSimpleName(), type);
     ...
     * 兼容 1.21.10 中 Recipe.getResult(HolderLookup.Provider) 方法。
     */
    private ItemStack getRecipeResult(Object recipe, Object registryManager) {
        // 尝试 getResult(HolderLookup.Provider)
        try {
    ...
    private static Holder<Item> pickBaseDyeableIngredient(List<ItemStack> stacks) {
        ...
        return stacks.getFirst().getItem().builtInRegistryHolder();
    }
